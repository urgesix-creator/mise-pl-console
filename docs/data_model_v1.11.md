# KOGAホールディングス海外飲食店 売上管理システム データモデル設計書 v1.11

| 項目 | 内容 |
|---|---|
| プロジェクト名 | KOGAホールディングス海外飲食店 売上管理システム |
| ドキュメント | データモデル設計書 |
| バージョン | v1.11（**販管費の上位分類をマスタ化**。固定8区分の CHECK 制約を廃止し、`expense_categories`（全社共通マスタ）を新設、`expense_accounts.level1` を `category_id`（FK）へ移行。マイグレーション010で実施・本番適用済み。§2.1・§4.9・§4.9.1 を実装実態に更新。**経営データ（daily_sales）・計算ロジック §8.1 は不変**） |
| 作成日 | 2026年5月10日（v1.7改訂：2026年5月30日／v1.8改訂：2026年5月30日／v1.9改訂：2026年5月30日／v1.10改訂：2026年5月31日／v1.11改訂：2026年6月2日） |
| 作成者 | 比嘉俊一（KOGAホールディングス株式会社 専務取締役） |
| 関連文書 | 要件定義書 v2.9 |
| 旧版 | v1.10（Excel往復「統合フォーマット」注記。要件定義書 v2.7 と整合） |

---

> **【v1.11 改訂サマリー（v1.10 → v1.11）】**
>
> **販管費の上位分類（旧 `level1`）を固定列挙からマスタ管理へ移行**した。**経営データ（`daily_sales` ほか取引系）の定義・計算 §8.1 は一切変更していない**。
>
> 1. **新マスタ `expense_categories` を追加**（§4.9.1）。全社共通（`store_id` を持たない）。列：`id` / `name` / `display_order` / `is_active` / `created_at` / `updated_at`。`name` は UNIQUE。
> 2. **`expense_accounts` を実装実態に更新**（§4.9）。旧設計列 `account_name` / `level1_category` / `level2_category` は実装に存在せず、実カラムは `name`（TEXT）/ `category_id`（FK → `expense_categories.id`, **ON DELETE RESTRICT**）。旧 `level1`（CHECK制約8区分固定）は**廃止**。
> 3. **マイグレーション010（`expense_categories_master`）で実施・本番適用済み**：8区分を投入 → `category_id` 追加 → 既存19行を旧 `level1` 名でバックフィル → NULL残ゼロ検証 → NOT NULL/FK 付与 → 旧 `level1`（列・CHECK・index）削除。`daily_expenses` は0件・経営データ非接触。
> 4. **上位分類は運用で自由に増減・改名・並び替え・無効化（ソフト削除）可能**。改名はマスタ1行更新で全科目に反映。使用中の区分は FK の RESTRICT で物理削除から保護。
> 5. **書き込み権限**：`expense_categories` は全社共通マスタのため **executive＋accounting**（RLS `expense_categories_write_admin`、`exchange_rates` と同パターン）。読取は全認証ユーザー。
> 6. **科目側の上位分類は登録後変更不可**（区分の改名はマスタ側で全科目に反映）。`expense_accounts` の店舗別追加・並び替え・無効化の既存仕様は不変。
>
> 詳細は §4.9・§4.9.1 を参照。
...
| 9.1 | expense_categories | 販管費上位分類マスタ（**全社共通・store_idなし**・v1.11追加） |

> **【v1.11 注記】** 基幹マスタは `expense_categories` 追加により実質10テーブル。`expense_categories` は全社共通（店舗非依存）のため番号は便宜上 9.1 とし、以降（取引系10〜）の採番は据え置く。
...
【v1.11追加リレーション】
- expense_categories (全社共通) ──→ expense_accounts.category_id
  （上位分類マスタ。FK: ON DELETE RESTRICT。expense_accounts は store_id=NULL で全店共通科目も保持）
```
...
> **【v1.11 改訂：実装実態に整合】** 旧設計の `account_name` / `level1_category` / `level2_category` は**実装に存在しない**。実カラムは `name`（TEXT）と `category_id`（上位分類マスタへの FK）。旧 `level1`（CHECK制約で8区分固定）は v1.11 で**廃止**し `category_id` へ移行（マイグレーション010）。

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---|---|---|---|---|
| id | UUID | NO | uuid_generate_v4() | 主キー |
| store_id | UUID | YES | - | FK to stores（**NULL = 全店共通科目**）。ON DELETE CASCADE |
| name | TEXT | NO | - | 科目名（例：給与、家賃、電気代、Google広告） |
| category_id | UUID | NO | - | **上位分類への FK → expense_categories.id（ON DELETE RESTRICT）**。旧 `level1` を置換 |
| display_order | INTEGER | NO | 0 | 表示順（同一スコープ・同一上位分類内） |
| is_active | BOOLEAN | NO | true | 有効フラグ（ソフト削除） |
| created_at, updated_at | TIMESTAMPTZ | NO | now() | 標準タイムスタンプ |

**ユニーク制約：** UNIQUE (store_id, name)

**インデックス：** (store_id) WHERE is_active、(category_id)

**スコープと権限：**
- `store_id IS NULL` は**全店共通科目**（経営層のみ書込可）。`store_id` 指定は店舗別追加科目（executive / country_rep / store_manager がアクセス可能店舗に対して書込）。
- **上位分類は登録後変更不可**（区分の改名は `expense_categories` 側で全科目に反映）。並び替えは「同一スコープ・同一 `category_id`・同一種別（共通/店舗別）」の有効レコード内のみ。

### 4.9.1 expense_categories（販管費上位分類マスタ・v1.11追加）

販管費科目の**上位分類**を保持する**全社共通マスタ**（店舗非依存・`store_id` を持たない）。旧 `expense_accounts.level1` の固定8区分（CHECK制約）を置き換える。運用で自由に**増減・改名・並び替え・無効化（ソフト削除）**できる。

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---|---|---|---|---|
| id | UUID | NO | gen_random_uuid() | 主キー |
| name | TEXT | NO | - | 上位分類名（**UNIQUE**）。改名はこの1行更新で全科目に反映 |
| display_order | INTEGER | NO | 0 | 表示順（科目マスタ画面のアコーディオン順） |
| is_active | BOOLEAN | NO | true | 有効フラグ（ソフト削除。無効区分は新規科目の選択肢から除外。既存科目・過去データは保持） |
| created_at, updated_at | TIMESTAMPTZ | NO | now() | 標準タイムスタンプ |

**ユニーク制約：** UNIQUE (name)

**初期データ（マイグレーション010で投入。旧8区分を踏襲）：**
...
**権限（RLS）：** 読取＝全認証ユーザー／書込（INSERT/UPDATE/DELETE）＝**executive＋accounting**（ポリシー `expense_categories_write_admin`、`exchange_rates` と同パターン）。

**参照保護：** `expense_accounts.category_id` からの FK は **ON DELETE RESTRICT**。使用中の区分は物理削除できない（運用上は無効化＝ソフト削除を使用）。
