# KOGAホールディングス海外飲食店 売上管理システム データモデル設計書 v1.8

| 項目 | 内容 |
|---|---|
| プロジェクト名 | KOGAホールディングス海外飲食店 売上管理システム |
| ドキュメント | データモデル設計書 |
| バージョン | v1.8（部門別売上（参考データ）機能を追加：sales_departments / daily_department_sales の2テーブルを新設。経営データ（daily_sales）からは物理分離し、損益・予算比に一切流入しない参考レイヤー。構成比（部門内シェア・累計）の集計ロジックを規定） |
| 作成日 | 2026年5月10日（v1.7改訂：2026年5月30日／v1.8改訂：2026年5月30日） |
