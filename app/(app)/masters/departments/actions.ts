...
function translateDbError(error: { code?: string; message: string }): string {
  if (error.code === '23505') return 'この部門名は既に存在します';
  return `処理に失敗しました: ${error.message}`;
}

export async function createDepartment(input: DepartmentFormData): Promise<ActionResult> {
  ...
  const supabase = await createClient();
  const { error } = await supabase.from('sales_departments').insert({
